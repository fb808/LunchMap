...
function handleRefresh() {
    let requestURL = 'http://openapi.seoul.go.kr:8088/424d44677566627739324d68534f71/JSON/CrtfcUpsoInfo/1/1000/ / / /3220000';
...
function NearbyRestaurant(restaurantInfo) {
    ...
    // JSON 파일 받기
    let info = restaurantInfo.CrtfcUpsoInfo.row;

    for (var i = 0; i < info.length; i++){
        // 필요한 정보만
        var obj = {
            title: info[i].UPSO_NM,
            latlng: new kakao.maps.LatLng(info[i].Y_DNTS, info[i].X_CNTS), // 위경도로 카카오맵 api 마커 생성에 필요한 객체 생성
            distance: computeDistance({latitude: info[i].Y_DNTS, longitude: info[i].X_CNTS}), // 거리 구하기
            address: info[i].RDN_CODE_NM,
            classification: info[i].BIZCND_CODE_NM
        };
